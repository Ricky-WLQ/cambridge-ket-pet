...
describe("<PortalMap>", () => {
  ...
  it("places each building at its placement coordinates in viewBox space", () => {
    ...
    const svgImages = container.querySelectorAll("svg image");
    const reading = svgImages[1] as SVGImageElement;
    expect(reading.getAttribute("x")).toBe("8");
    expect(reading.getAttribute("y")).toBe("38");
    expect(reading.getAttribute("width")).toBe("28");
    expect(reading.getAttribute("height")).toBe("28");
  });
